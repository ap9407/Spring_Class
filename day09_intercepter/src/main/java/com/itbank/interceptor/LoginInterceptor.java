...
// 스프링 인터셉터 : 요청을 가로채서, 사전 코드를 실핼할 수 있다. 여러 주소에 대해 일괄 적용 가능
// Handler : 컨트롤러의 메서드를 나타낸다

// 1) preHandler : 요청이 컨트롤러에 도달하기 전에 실행되는 함수(DispatcherServlet -> controller)
// 2) postHandler : 컨트롤러의 메서드가 수행된 이후 실행되는 함수(return mav 이후)
...
public class LoginInterceptor extends HandlerInterceptorAdapter{
	...
	// 즉, preHandler 의 반환값은 예정대로 진행할 것인가? 에 대한 답이다.
